...
export class KalmanFusion extends SensorFusion {
  constructor(device) {
    super(device);
  ...
  predict() {
    let gyroVal = this.device.hasGyroscope() ? this.device.getGyroscope().val() : new p5.Vector(0, 0, 0);
    if (!gyroVal) {
      gyroVal = new p5.Vector(0, 0, 0);
    }
