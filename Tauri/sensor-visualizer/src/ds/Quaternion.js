...
class Quaternion {
  ...
  projXZ() {
    const rot = this.multVec(new Vector(0.0, 1.0, 0.0));
    const result = new Vector(rot.x, rot.z);
    // Validate result
  ...
  projYX() {
    const rot = this.multVec(new Vector(1.0, 0.0, 0.0));
    const result = new Vector(rot.y, rot.x);
    // Validate result
    if (!isFinite(result.x)) result.x = 0;
    if (!isFinite(result.y)) result.y = 0;
    return result;
  ...
  projZY() {
    const rot = this.multVec(new Vector(0.0, 0.0, 1.0));
    const result = new Vector(rot.z, rot.y);
    // Validate result
    if (!isFinite(result.x)) result.x = 0;
    if (!isFinite(result.y)) result.y = 0;
    return result;
